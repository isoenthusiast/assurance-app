...
export default function ProcessDetailsClient({
  ...
  const handleSaveControl = async () => {
    ...
    try {
      ...
      const toDelete = existingLinks.filter((l) => !linkedSubProcessIds.has(l.subProcessId));
      // Create new links
      const toCreate = Array.from(linkedSubProcessIds).filter((spId) => !existingBySubProcess.has(spId));

      for (const link of toDelete) {
        await fetch(`/api/admin/table/ControlSubProcess/${link.id}`, { method: "DELETE" }).catch(() => {});
      }
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-6">
            ...
            {assessments
              ...
              .flatMap((a) =>
                a.findings
                  .filter((f) => f.actions.some(act => !act.actionClosureEffective))
                  .filter((f) => {
                    // Filter findings linked to controls in this process area
                    const linkedControlIds = (f.controlIds || "").split(",").map(s => s.trim()).filter(Boolean);
                    return linkedControlIds.some(cid => allControls.some(c => c.id === cid));
                  })
              ...
              <div className="space-y-4">
                {assessments
                  ...
                  .slice(0, 10)
                  .map((item) => (
                    <div key={item.finding.id} className="border border-slate-200 rounded-lg overflow-hidden">
                      {/* Finding header */}
                      <div className="flex items-center justify-between bg-slate-50 px-4 py-3 border-b border-slate-200">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-medium text-slate-900">{item.finding.description}</span>
                          <span className="text-xs text-slate-500">{item.assessmentName}</span>
                        </div>
                        <Link href={`/fla/${item.assessmentId}`} className="text-xs text-blue-600 hover:underline">View</Link>
                      </div>
                      {/* Actions subtable */}
                      <table className="w-full text-xs">
                        <thead className="bg-slate-100">
                          <tr>
                            ...
                            <th className="px-4 py-2 text-left font-medium text-slate-600">Target Date</th>
                            <th className="px-4 py-2 text-left font-medium text-slate-600">Status</th>
                          </tr>
                        </thead>
                        <tbody>
                          {item.finding.actions.filter(a => !a.actionClosureEffective).map((action) => (
